...
TEST_F(PCTest, PCIncrements) {
    tick();
    EXPECT_EQ(top->pccur, 4u);

    tick();
    EXPECT_EQ(top->pccur, 8u);

    tick();
    EXPECT_EQ(top->pccur, 12u);
}
...
TEST_F(PCTest, BranchForward) {
    ...
    tick();
    EXPECT_EQ(top->pccur, 16u);

    top->pcsrc = 0;
    tick();
    EXPECT_EQ(top->pccur, 20u);
}
